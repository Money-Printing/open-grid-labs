...
export default function WorkingStep({ steps }: WorkingStepProps) {
	return (
		<section id="service-details-working-step" className="w-full flex flex-col items-center relative py-20">
			...
			<div className="w-full w-[90%] max-w-[1600px] grid md:grid-cols-2 grid-cols-1 gap-10 mt-16 px-4 md:px-8 z-10">
				{steps.map((step, idx) => (
					<div
						key={idx}
					...
					>
						...
						<h2 className="font-bold text-2xl mt-2 ml-4 text-foreground drop-shadow-sm">{step.title}</h2>
						<p className="text-muted-foreground text-lg ml-4 leading-relaxed group-hover:text-foreground/80 transition-colors">{step.desc}</p>
					</div>
				))}
			</div>
		</section>
